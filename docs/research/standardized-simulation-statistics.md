...
Research date: 2026-09-05. Updated for accepted product decisions: 2026-09-06. Evidence answer for [Standardized strategy simulation statistics](https://github.com/0xRowdy/true-count/issues/18), supporting [Community publication, rankings, and results](https://github.com/0xRowdy/true-count/issues/6).
...
The following combines accepted product decisions with application details derived from those methods. Statistical run sizes and precision thresholds remain proposals.

The user accepted seven launch benchmark profiles: the four blackjack combinations of H17/S17 and late surrender on/off under the accepted blackjack rules, default standard craps, and separate single-zero and double-zero roulette profiles. Each starts with **1,000 simulated currency units**, a **10-unit base bet**, and **200 table opportunities**. Thus `B = 1000`, `u = 10`, and the initial bankroll is 100 base units. Outstanding commitments settle after the horizon. A profile must also pin complete table conditions, initialization, settlement details, and any benchmark participation restrictions. Record strategy revision, engine version, analysis-protocol version, RNG algorithm, stream allocation, and run identifier. Compare matching profiles and compatible engine/protocol versions only.

The accepted horizon counts **table opportunities**, not the strategy's resolved wagers: blackjack rounds, craps rolls, roulette spins. A player sitting out must not freeze the horizon; the profile must define how the table and observable information advance during abstention. Different games have different opportunity meanings and remain separate comparisons. Starting-shoe/table conventions and the mechanics of table advancement during abstention still require specification.
...
**Settlement can exceed the horizon.** In ordinary craps, a contract waiting for a point or seven can encounter arbitrarily many other rolls; a finite new-bet horizon does not imply a deterministic runtime bound. The accepted policy closes discretionary betting and resolves existing commitments. It must specify removable wagers, working status, and whether any settlement-time actions are obligatory. A watchdog resumes the same unfinished session later or marks the entire run incomplete. It cannot silently drop long sessions, replace their seeds, count them as zero, or treat a forced refund as an ordinary result. A hard tail cutoff with censoring would define a different estimator and needs a separate design.

Finite bankroll alone bounds losses, not necessarily gains. Before relying on finite-moment approximations, establish bounds on opportunities, aggregate stakes, payout multipliers, split/continuation exposure, and the number of remaining commitments. Under those restrictions and no new tail bets, monetary results can be bounded even when settlement time is unbounded. The accepted bankroll and horizon do not discharge this engine proof obligation. The capped domain, payout/exposure bounds, and adequate coverage of the approximate mean and ratio intervals must be validated separately; none is a proved property of the current app.
...
The accepted exhaustion definition is **exactly zero settled balance**. Show it separately from stopping because the next prescribed action is unaffordable. A player can have money left and still be unable to continue a progression. Keep progression caps and strategy stops separate. Label these as probabilities **within this profile**, not lifetime risk of ruin.
...
The user accepted equity with **outstanding stakes valued at cost**, with all simultaneous settlements processed atomically. Define `E_t = cash available at t + cost of outstanding stakes at t`, equivalently `B + cumulative net settled profit at event t` when cash transfers and wager costs reconcile. Placing a wager transfers its cost from available cash to outstanding stakes without creating a loss; settlement replaces that cost with its actual proceeds. Aggregate simultaneous settlements before updating the curve so arbitrary bookkeeping order cannot create peaks. This records drawdown in equity at cost; affordability still depends on available cash.

For each session, `D_i = max_t(max_(s<=t) E_s - E_t) / u`, including final settlement. The accepted summaries are median, 95th percentile, and observed worst drawdown. For the median and 95th percentile, propose 95% order-statistic confidence intervals chosen using exact binomial ranks; use a documented inverse empirical-CDF quantile and conservative handling of ties. Do not confuse the 95th percentile of drawdown with 95% confidence in its estimate. [NIST TN 2119, §5.3](https://nvlpubs.nist.gov/nistpubs/TechnicalNotes/NIST.TN.2119.pdf)
...
Run counts, confidence levels, and precision thresholds in this section are **proposed product choices**, not empirical findings or performance promises. The profile amounts and 200-opportunity horizon above are already accepted.
...
This resolves the research question with a candidate method and explicit limits. The seven profiles, initial bankroll/base unit, horizon, final settlement, exhaustion definition, and drawdown convention/summaries are accepted as recorded above. This does not authorize the proposed 100,000-session run size or precision thresholds, or certify launch comparisons. Product decisions remain: initialization and remaining settlement details, participation restrictions, precision tolerances, and when a public result can appear in discovery.
